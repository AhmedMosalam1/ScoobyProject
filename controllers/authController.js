...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const result = await userModel.findOne({ email }).populate('pets').populate('services_id')
...
<body>
  ...
  try {
    ...
    return next(
      new appError(`There is an error in sending email ${err} `, 500)
    );
  }
...
exports.getresetpass = catchAsync(async (req, res, next) => {
  ...
  if (req.body.password !== req.body.confirmPassword) {
    return next(
      new appError("Password and confirmation password do not match", 400)
    );
  }
